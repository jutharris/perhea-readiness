...
const OnboardingEducation: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950 z-50 flex flex-col overflow-hidden">
      {/* Progress Bars */}
      <div className="flex gap-1 px-4 pt-6 pb-4">
        {CONCEPTS.map((_, idx) => (
          ...
          >
            ...
            <div className="space-y-6 flex-1">
              <div className="space-y-2">
                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Concept {currentStep + 1} • {concept.duration}</p>
                <h2 className="text-3xl font-black text-white italic uppercase leading-tight tracking-tight">
                  {concept.header}
                </h2>
              </div>

              <div className="text-slate-400 font-medium leading-relaxed text-lg whitespace-pre-wrap">
                {concept.body}
              </div>
            </div>
          </motion.div>
